<?php
	...
	if(isset($_POST["register"]))
	{
		...
		if(!isset($error)){
		if(mysqli_query($connection, "INSERT INTO `photographer`(`firstName`, `lastName`, `mobileNumber`, `password`, `email`, `profile`, `about`, `status`) VALUES('" . ucwords(strtolower(trim($connection,$_POST["firstName"]))) . "', '" . ucwords(strtolower(trim($connection,$_POST["lastName"]))) . "', '" . trim($connection,$_POST["mobileNumber"]) . "', '" . trim($connection,$_POST["password"]) . "', '" . trim($connection,$_POST["email"]) . "', '$fileName1', '" . trim($connection,$_POST["about"]) . "', 'OFF')"))
		{
...
<body class="royal_preloader">	
	...
	<div class="section-block padding-top-small padding-bottom">
		...
		?>
			...
			<div class="six columns">
				<label for="profile"><img id="profileImage" src="images/noimage.jpg" width="100%" style="cursor:pointer" /></label>
