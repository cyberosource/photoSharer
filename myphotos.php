<?php
	...
	if(isset($_POST["upload"]))
	{
		...
		if(!empty($fileName1))
		{
			...
			$fileName1 = "PHOTO-SHARE-PHOTOGRAPHER-IMAGE-" . time() . "." . $fileExt1;
			move_uploaded_file($fileTmpLoc1, "images/portfolio/$fileName1");
		}
		if(!isset($error)){
		if(mysqli_query($connection, "INSERT INTO `photos`(`category`, `photosName`, `image`, `price`, `photographerId`) VALUES('" . ucwords(strtolower(trim($connection,$_POST["category"]))) . "', '" . ucwords(strtolower(trim($connection,$_POST["photosName"]))) . "', '$fileName1', '" . trim($connection,$_POST["price"]) . "', '" . $_SESSION["photographerId"] . "')"))
		{
			header("Location:myphotos.php?success=Your Photo's Upload Successfully.");
		}
		}
	}
?>
...
<body class="royal_preloader">	
	...
	<div class="section-block padding-top-small padding-bottom">
		...
		?>
			<form name="ajax-form" id="ajax-form" method="post" enctype="multipart/form-data">
				<div class="six columns">
					<input name="category" id="category" type="text" placeholder="Enter Image Category *" value="<?php
					echo isset($_POST["category"]) ? $_POST["category"] : "";
					?>" required />
				</div>
				<div class="clear"></div>
				<div class="six columns">
					<input name="photosName" id="photosName" type="text" placeholder="Enter Image Name *" value="<?php
					echo isset($_POST["photosName"]) ? $_POST["photosName"] : "";
					?>" required />
				</div>
				<div class="clear"></div>
				<div class="six columns">
					<input name="price" id="price" type="text" placeholder="Enter Image Price *" value="<?php
					echo isset($_POST["price"]) ? $_POST["price"] : "";
					?>" required />
				</div>
				<div class="clear"></div>
				<div class="six columns">
					<input name="image" id="image" type="file" value="<?php
					echo isset($_POST["image"]) ? $_POST["image"] : "";
					?>" required />
				</div>
				<div class="clear"></div>
				<div class="six columns remove-top">
					<div id="button-con"><button class="send_message button-effect button--moema button--text-thick button--text-upper button--size-s" id="upload" name="upload" data-lang="en">Upload</button></div>
				</div>
				<div class="clear"></div>	
			</form>
		</div>
		...
		<table class="table-fill">
			...
			<?php
			$psr = 1;
			$resultPhoto = mysqli_query($connection, "SELECT * FROM `photos` WHERE `photographerId` = '" . $_SESSION["photographerId"] . "' ORDER BY `photosId` DESC");
			...
			?>
				<tr>
					...
					<td class="text-left">
						<a href="?photosIdDelete=<?php echo $rowPhoto["photosId"]; ?>" onclick="javascript:return confirm('Are You Sure You Want To Remove This Image?');return false;"><img src="images/delete.ico" style="max-width:50px;min-width:50px;max-height:50px;min-height:50px;" /></a>
					</td>
				</tr>
			<?php
			}
